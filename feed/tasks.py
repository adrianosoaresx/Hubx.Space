...
def notificar_autor_sobre_interacao(post_id: str, tipo: str) -> None:
    try:
        ...
        return
    event = resolve_interaction_template(tipo)
    context = build_interaction_notification_context(post_id=str(post.id), interaction_type=tipo)
    try:
        enviar_para_usuario(post.autor, event, context)
