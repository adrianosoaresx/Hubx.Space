...
def _dispatch_connection_notification(user, event_key: str, actor_user) -> float:
    start = time.perf_counter()
    template_codigo = resolve_connection_template(event_key)
    context = build_connection_notification_context(
        event_key=event_key,
        actor_display_name=_get_display_name(actor_user),
...
def aceitar_conexao(request, id):
    ...
    _dispatch_connection_notification(other_user, "accepted", request.user)
    if is_htmx_or_ajax(request):
        return _htmx_refresh_response()
    return redirect(redirect_url)
...
def recusar_conexao(request, id):
    ...
    _dispatch_connection_notification(other_user, "declined", request.user)
    if is_htmx_or_ajax(request):
        return _htmx_refresh_response()
    return redirect(redirect_url)
