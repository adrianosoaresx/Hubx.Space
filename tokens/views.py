...
from apps.backend.app.modules.tokens.application.invite_flow import (
    build_invite_totals as _build_invite_totals_uc,
    get_query_param as _get_query_param_uc,
    redirect_root_from_tokens as _redirect_root_from_tokens_uc,
)
from apps.backend.app.modules.tokens.application.token_validation import (
    validate_event_token_compatibility as _validate_event_token_compatibility_uc,
)
...
def _redirect_root_from_tokens(request):
    return _redirect_root_from_tokens_uc(request)


def _get_query_param(request, key: str):
    return _get_query_param_uc(request, key)
...
def token(request):
    ...
    def _handle_token_success(token_obj, token_code):
        error_message = _validate_event_token_compatibility_uc(evento, token_obj)
        if error_message:
...
def listar_convites(request):
    ...
    totais = _build_invite_totals_uc(convites_qs)
