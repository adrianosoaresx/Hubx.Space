# Smoke Onda 3.3 - Pagamentos Checkout Execution

Data: 2026-03-10 17:19:45
...
Escopo: validar extração da execução de pagamento (provider/service + tratamento de erro de domínio) para `application`.
...
| pagamentos_pix_checkout_redirect | `/pagamentos/checkout/resultado/47/` | OK | Rota de resultado gerada corretamente |
| pagamentos_transacao_criada | `Transacao(id=47)` | OK | `status=approved`, `metodo=pix`, `pedido.organizacao` preservada |
...
Comando executado:
- `./.venv/Scripts/python.exe manage.py shell -c "<script smoke checkout execution>"`

Observacao operacional:
- O smoke utilizou stub local de `PagamentoService.iniciar_pagamento` para validar o fluxo interno sem dependência externa de provider.
