# Smoke Onda 3.3 - Pagamentos Pix Checkout Use Case

Data: 2026-03-10 17:24:20
...
Escopo: validar unificação do checkout Pix em use-case único na camada `application`.
...
| pagamentos_pix_checkout_redirect | `/pagamentos/checkout/resultado/48/` | OK | Rota de resultado gerada corretamente |
| pagamentos_transacao_criada | `Transacao(id=48)` | OK | `status=approved`, `metodo=pix`, `pedido.organizacao` preservada |
...
Comando executado:
- `./.venv/Scripts/python.exe manage.py shell -c "<script smoke pix checkout use case>"`

Observacao operacional:
- O smoke utilizou stub local de `PagamentoService.iniciar_pagamento` para validar apenas o fluxo interno unificado sem dependência externa.
