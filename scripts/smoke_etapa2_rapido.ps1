...
$historyPath = "docs/etapas/smoke_etapa2_rapido_history.md"
$runStatus = "SUCCESS"
...
if not target:
    target = User.objects.create_user(
        username=f"smoke_target_rapido_{user.id}",
        email=f"smoke_target_rapido_{user.id}@hubx.local",
        password="Smoke@123",
...
if not nucleo:
    nucleo = Nucleo.objects.create(organizacao=org, nome=f"Smoke Rapido Nucleo {str(org.id)[:8]}")
...
def _fake_iniciar_pagamento(self, pedido, metodo, dados_pagamento):
    return Transacao.objects.create(
        ...
        metodo=metodo,
        detalhes={"status": "approved", "baseline_rapido_script": True},
    )
...
'@
    ...
    if (-not (Test-Path $historyPath)) {
        "# Historico Smoke Etapa 2 Rapido`n" | Out-File -FilePath $historyPath -Encoding utf8
    }
    $entry = @()
    $entry += "## $runStamp"
    $entry += "- status: $runStatus"
    $entry += "- script: scripts/smoke_etapa2_rapido.ps1"
    $entry += "- etapas:"
    $entry += $stepResults
    $entry += ""
    ...
    if ($runStatus -eq "SUCCESS") {
        Write-Host "Smoke rapido da Etapa 2 concluido com sucesso."
    }
    else {
        Write-Error "Smoke rapido da Etapa 2 finalizou com falha."
        exit 1
    }
}
