...
# Security / Core
# -----------------------------------------------------------------------------
# Usa variável de ambiente no CI/prod; mantém fallback para desenvolvimento local
SECRET_KEY = os.getenv(
    "DJANGO_SECRET_KEY",
    "django-insecure-f_(^m#wo6a9xfpmwi)8n7bo@k@gs!sp_(9-=jhi0%2r6o@2i^8",
)
...
# Lê listas a partir de variáveis do GitHub Actions (separadas por vírgula)
...
# -----------------------------------------------------------------------------
# Channels (Redis quando REDIS_URL existir; senão, fallback em memória)
# -----------------------------------------------------------------------------
...
if _database_url:
    try:
        ...
        # Se dj_database_url não estiver presente, mantém SQLite (CI/local)
        pass

# -----------------------------------------------------------------------------
# Cache (Redis se REDIS_URL/CACHE_URL informado; senão LocMem)
# -----------------------------------------------------------------------------
CACHE_URL = os.getenv("CACHE_URL") or REDIS_URL
if CACHE_URL:
    CACHES = {
        "default": {
...
else:  # fallback para desenvolvimento/CI sem Redis
    CACHES = {
        "default": {
...
# -----------------------------------------------------------------------------
# Notificações (stubs seguros por padrão)
# -----------------------------------------------------------------------------
